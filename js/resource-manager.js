 ...
 */
    ...
    function calculateResourceLoad(tasks, resources, assignments) {
        ...
        resources.forEach(res => {
            // Normalize maxUnits: if > 10, treat as percentage (e.g. 100 = 100%)
            const unitsFraction = (res.maxUnits || 1) > 10 ? (res.maxUnits / 100) : (res.maxUnits || 1);
            loads[res.uid] = {
                resource: res,
                dailyLoad: {},  // date -> hours
                totalHours: 0,
                maxDailyHours: unitsFraction * 8
            };
        });
        ...
        assignments.forEach(asgn => {
            ...
            // Normalize assignment units: if > 10, treat as percentage
            const unitsFrac = (asgn.units || 1) > 10 ? ((asgn.units || 100) / 100) : (asgn.units || 1);
            const dailyHours = (unitsFrac * 8);
    ...
    function getTimeTrackingSummary(tasks, resources, assignments) {
        ...
        resources.forEach(res => {
            ...
            resAsgns.forEach(a => {
                ...
                // Normalize units: >10 means percentage (e.g. 100 = 100%)
                const unitsFrac = (a.units || 1) > 10 ? ((a.units || 100) / 100) : (a.units || 1);
                const taskPlanned = (t.durationDays || 0) * 8 * unitsFrac;
                plannedHours += t.plannedHours || taskPlanned;
                actualHours += t.actualHours || 0;
            });
            // Efficiency: actual/planned when actual exists, otherwise show completion-based estimate
            let efficiency = 0;
            if (actualHours > 0 && plannedHours > 0) {
                efficiency = Math.round((actualHours / plannedHours) * 100);
            } else if (plannedHours > 0) {
                // No actual hours tracked yet — show 0%
                efficiency = 0;
            }
            summary.push({
                ...
                variance: Math.round(actualHours - plannedHours),
                efficiency
            });
        });
        return summary;
