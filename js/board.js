...
// ══════════════════════════════════════════════════════════════
    ...
    function renderHeatmap(container, project, weeks) {
        // Auto-calculate weeks from project date range
        if (!weeks) {
            const pStart = project.startDate ? new Date(project.startDate) : new Date();
            const pEnd = project.finishDate ? new Date(project.finishDate) : new Date(pStart.getTime() + 84 * 86400000);
            weeks = Math.max(4, Math.ceil((pEnd - pStart) / (7 * 86400000)) + 1);
        }
        ...
        // Align to Sunday (Egypt calendar: Sun-Thu)
        const dayOfWeek = startDate.getDay();
        const daysToSun = dayOfWeek === 0 ? 0 : -(dayOfWeek);
        const weekStart = new Date(startDate);
        weekStart.setDate(weekStart.getDate() + daysToSun);
        ...
        (project.assignments || []).forEach(a => {
            ...
            weekStarts.forEach((ws, wi) => {
                ...
                if (overlapStart < overlapEnd) {
                    ...
                    loadArr[wi] += days * hoursPerDay * unitsFrac;
                }
            });
        });
