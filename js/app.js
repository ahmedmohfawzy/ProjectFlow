...
import { TeamsBridge } from './teams-bridge.js';
    ...
    function bindEvents() {
        ...
        _initSyncStatusUI();    // Sync status indicator in footer
        initD365Sync();         // D365 Project Accounting

        // ── Teams Bridge: Auto-connect Planner & D365 ─────────
        // Listen for Planner events — only log, user must manually open MS Planner Sync
        EventBus.on('planner:connected', ({ plans, lastPlanId }) => {
            if (!plans || plans.length === 0) return;
            console.log(`[PF] Planner plans available: ${plans.length}. Use "MS Planner Live Sync" to import.`);
        });
    ...
    function renderResourceHeatmap() {
        ...
        // Also render the histogram canvas
        renderResourceHistogram();
    }

    /**
     * Draw a stacked bar histogram on the histogramCanvas
     * showing daily resource load aggregated across all resources.
     */
    function renderResourceHistogram() {
        ...
        if (!resources.length || !assignments.length) {
            ...
            ctx.scale(dpr, dpr);
            ctx.clearRect(0, 0, canvas.clientWidth, 200);
            ctx.fillStyle = 'rgba(255,255,255,0.3)';
            ctx.font = '13px Inter, system-ui, sans-serif';
            ctx.textAlign = 'center';
            ctx.fillText('No resource assignments to display', canvas.clientWidth / 2, 100);
            return;
        }

        const { resourceLoads } = ResourceManager.calculateResourceLoad(
            project.tasks, resources, assignments
        );

        // Aggregate daily hours across all resources
        const dailyTotals = new Map(); // date → total hours
        let maxCapacity = 0;
        Object.values(resourceLoads).forEach(rl => {
            maxCapacity += rl.maxDailyHours;
            Object.entries(rl.dailyLoad).forEach(([date, hrs]) => {
                dailyTotals.set(date, (dailyTotals.get(date) || 0) + hrs);
            });
        });
        ...
        const values = sortedDates.map(d => dailyTotals.get(d));
        const maxVal = Math.max(...values, maxCapacity, 8);

        // Canvas setup — use parent's width (canvas clientWidth can be 0 before layout)
        const container = canvas.parentElement;
        const W = container?.clientWidth || canvas.clientWidth || 900;
        const H = 250;
        const dpr = window.devicePixelRatio || 1;
        canvas.width = W * dpr;
        canvas.height = H * dpr;
        ...
        const padL = 40, padR = 20, padT = 15, padB = 35;
        const chartW = W - padL - padR;
        const chartH = H - padT - padB;
        const barW = Math.max(3, Math.min(16, (chartW / sortedDates.length) - 1));
        const step = chartW / sortedDates.length;

        // Y axis scale
        const yScale = chartH / maxVal;

        // Draw capacity line
        const capY = padT + chartH - (maxCapacity * yScale);
        ...
        ctx.textAlign = 'left';
        ctx.fillText(`${maxCapacity}h capacity`, padL + 4, capY - 3);

        // Draw bars
        sortedDates.forEach((date, i) => {
            const hrs = values[i];
            const barH = hrs * yScale;
            const x = padL + i * step + (step - barW) / 2;
            const y = padT + chartH - barH;

            // Color based on allocation
            const ratio = hrs / maxCapacity;
            if (ratio > 1) {
                ctx.fillStyle = 'rgba(239, 68, 68, 0.85)'; // Over-allocated: red
            } else if (ratio > 0.75) {
                ctx.fillStyle = 'rgba(245, 158, 11, 0.85)'; // High: amber
            } else if (ratio > 0) {
                ctx.fillStyle = 'rgba(99, 102, 241, 0.75)'; // Normal: indigo
            } else {
                ctx.fillStyle = 'rgba(100, 116, 139, 0.3)'; // Idle: gray
            }
        ...
        ctx.textAlign = 'center';
        const labelInterval = Math.max(1, Math.ceil(sortedDates.length / 12));
        sortedDates.forEach((date, i) => {
    ...
    function renderResources() {
        ...
        for (const res of project.resources) {
            ...
            addCell(tr, (res.type === 1 || res.type === 'Work') ? 'Work' : 'Material');
            const mu = typeof res.maxUnits === 'number' ? (res.maxUnits > 10 ? res.maxUnits : res.maxUnits * 100) : 100;
            addCell(tr, mu + '%');
            addCell(tr, settings.currency + (res.costPerHour || res.cost || 0));
    ...
    function renderTimeTrackingSummary() {
        ...
        const utilSummary = ResourceManager.getUtilizationSummary(resourceLoads, project.startDate, project.finishDate);

        // === KPI CALCULATIONS ===
        const totalPlanned = summary.reduce((s, r) => s + r.plannedHours, 0);
        const totalActual = summary.reduce((s, r) => s + r.actualHours, 0);
        const avgUtil = utilSummary.length > 0 ? Math.round(utilSummary.reduce((s, u) => s + (u.utilization || 0), 0) / utilSummary.length) : 0;
        const overAllocCount = utilSummary.filter(u => u.overAllocated).length;
        const totalCost = summary.reduce((s, r) => {
            const res = project.resources.find(pr => pr.uid === r.uid);
            return s + (r.plannedHours * (res?.costPerHour || res?.cost || 0));
        }, 0);

        // Resource colors for charts
        const COLORS = ['#6366f1', '#8b5cf6', '#ec4899', '#f59e0b', '#22c55e', '#3b82f6', '#ef4444', '#14b8a6', '#f97316', '#a855f7'];

        let html = '';

        // === KPI CARDS ===
        html += '<div class="res-analytics-grid">';
        html += `<div class="res-kpi-card">
            <div class="res-kpi-icon" style="color:#6366f1">📊</div>
            <div class="res-kpi-value">${totalPlanned.toLocaleString()}h</div>
            <div class="res-kpi-label">Total Planned</div>
        </div>`;
        html += `<div class="res-kpi-card">
            <div class="res-kpi-icon" style="color:#22c55e">⏱️</div>
            <div class="res-kpi-value">${totalActual > 0 ? totalActual.toLocaleString() + 'h' : '—'}</div>
            <div class="res-kpi-label">Actual Logged</div>
        </div>`;
        html += `<div class="res-kpi-card">
            <div class="res-kpi-icon" style="color:#3b82f6">📈</div>
            <div class="res-kpi-value">${avgUtil}%</div>
            <div class="res-kpi-label">Avg Utilization</div>
        </div>`;
        html += `<div class="res-kpi-card ${overAllocCount > 0 ? 'res-kpi-danger' : ''}">
            <div class="res-kpi-icon" style="color:${overAllocCount > 0 ? '#ef4444' : '#22c55e'}">⚠️</div>
            <div class="res-kpi-value">${overAllocCount}</div>
            <div class="res-kpi-label">Over-allocated</div>
        </div>`;
        html += `<div class="res-kpi-card">
            <div class="res-kpi-icon" style="color:#f59e0b">💰</div>
            <div class="res-kpi-value">${totalCost > 0 ? settings.currency + totalCost.toLocaleString() : '—'}</div>
            <div class="res-kpi-label">Cost Forecast</div>
        </div>`;
        html += '</div>';

        // === CHARTS ROW ===
        html += '<div class="res-charts-row">';

        // Chart 1: Workload Distribution (Donut)
        html += '<div class="res-chart-card">';
        html += '<h4>Workload Distribution</h4>';
        html += '<canvas id="resDonutChart" width="220" height="220"></canvas>';
        html += '<div class="res-chart-legend" id="resDonutLegend"></div>';
        html += '</div>';

        // Chart 2: Resource Utilization Bars
        html += '<div class="res-chart-card res-chart-wide">';
        html += '<h4>Resource Utilization</h4>';
        html += '<canvas id="resUtilChart" width="500" height="220"></canvas>';
        html += '</div>';

        html += '</div>';

        // === TIME TRACKING TABLE (improved) ===
        html += '<h4 style="margin:20px 0 8px;color:var(--text-secondary);font-size:0.85rem">⏱️ Time Tracking Summary</h4>';
        html += '<table class="data-table" style="margin-bottom:12px"><thead><tr>';
        html += '<th>Resource</th><th>Planned</th><th>Actual</th><th>Variance</th><th>Efficiency</th><th>Workload</th>';
        html += '</tr></thead><tbody>';

        const maxPlanned = Math.max(...summary.map(s => s.plannedHours), 1);
        for (const s of summary) {
            const varSign = s.variance > 0 ? '+' : '';
            const varClass = s.variance > 0 ? 'color:#ef4444' : s.variance < 0 ? 'color:#f59e0b' : 'color:#22c55e';
            const effText = s.actualHours > 0 ? `${s.efficiency}%` : '<span style="color:var(--text-muted)">—</span>';
            const barPct = Math.round((s.plannedHours / maxPlanned) * 100);
            const color = COLORS[summary.indexOf(s) % COLORS.length];

            html += '<tr>';
            html += `<td><span style="display:inline-block;width:8px;height:8px;border-radius:50%;background:${color};margin-right:6px"></span>${escapeHTML(s.name)}</td>`;
            html += `<td>${s.plannedHours}h</td>`;
            html += `<td>${s.actualHours > 0 ? s.actualHours + 'h' : '<span style="color:var(--text-muted)">—</span>'}</td>`;
            html += `<td style="${varClass}">${s.actualHours > 0 ? varSign + s.variance + 'h' : '<span style="color:var(--text-muted)">—</span>'}</td>`;
            html += `<td>${effText}</td>`;
            html += `<td style="min-width:120px"><div style="display:flex;align-items:center;gap:6px"><div style="flex:1;height:6px;background:var(--bg-active);border-radius:3px;overflow:hidden"><div style="width:${barPct}%;height:100%;background:${color};border-radius:3px"></div></div><span style="font-size:0.68rem;color:var(--text-muted)">${barPct}%</span></div></td>`;
            html += '</tr>';
        }
        html += '</tbody></table>';
        ...
        // === DRAW DONUT CHART ===
        requestAnimationFrame(() => {
            _drawDonutChart(summary, COLORS);
            _drawUtilChart(utilSummary, COLORS);
        });
    }

    function _drawDonutChart(summary, colors) {
        const canvas = $('resDonutChart');
        if (!canvas) return;
        const ctx = canvas.getContext('2d');
        const dpr = window.devicePixelRatio || 1;
        const W = 220, H = 220;
        ...
        const total = summary.reduce((s, r) => s + r.plannedHours, 0) || 1;
        const cx = W / 2, cy = H / 2, outerR = 90, innerR = 55;
        let angle = -Math.PI / 2;

        // Draw segments
        summary.forEach((s, i) => {
            const sliceAngle = (s.plannedHours / total) * Math.PI * 2;
            ctx.beginPath();
        ...
        ctx.fillText(`${total}h`, cx, cy - 8);
        ctx.font = '10px Inter, system-ui, sans-serif';
        ctx.fillStyle = 'rgba(255,255,255,0.5)';
        ctx.fillText('Total Hours', cx, cy + 10);

        // Build legend
        const legend = $('resDonutLegend');
        if (legend) {
            legend.innerHTML = summary.map((s, i) => {
                const pct = Math.round((s.plannedHours / total) * 100);
                return `<span class="res-legend-item"><span class="res-legend-dot" style="background:${colors[i % colors.length]}"></span>${s.name} (${pct}%)</span>`;
            }).join('');
        }
    }

    function _drawUtilChart(utilSummary, colors) {
        const canvas = $('resUtilChart');
        if (!canvas) return;
        const ctx = canvas.getContext('2d');
        const dpr = window.devicePixelRatio || 1;
        const W = canvas.clientWidth || 500, H = 220;
        canvas.width = W * dpr; canvas.height = H * dpr;
        canvas.style.width = W + 'px'; canvas.style.height = H + 'px';
        ctx.scale(dpr, dpr);

        const padL = 10, padR = 10, padT = 15, padB = 50;
        const chartW = W - padL - padR;
        const chartH = H - padT - padB;
        const barCount = utilSummary.length || 1;
        ...
        ctx.font = '9px Inter, system-ui, sans-serif';
        ctx.textAlign = 'left';
        ctx.fillText('100%', padL + 2, y100 - 3);

        // Draw bars
        utilSummary.forEach((u, i) => {
            const val = Math.min(u.utilization || 0, maxVal);
            ...
            ctx.textAlign = 'center';
            ctx.fillText(`${u.utilization || 0}%`, x + barW / 2, y - 5);
            ...
            ctx.font = '9px Inter, system-ui, sans-serif';
            ctx.textAlign = 'right';
            const name = (u.name || '').length > 12 ? (u.name || '').substring(0, 12) + '…' : (u.name || '');
            ctx.fillText(name, 0, 0);
            ctx.restore();

            // Over-allocation glow
            if ((u.utilization || 0) > 100) {
    ...
    function _initSyncStatusUI() {
        const el = $('syncStatus');
        if (!el) return;

        // Clicking the badge opens the sync panel directly
        el.addEventListener('click', () => openPlannerSyncModal());

        window.addEventListener('pf-sync', (e) => {
            const { type, detail } = e.detail || {};
            el.classList.remove('hidden', 'synced', 'syncing', 'error');

            switch (type) {
                case 'started':
                    el.classList.add('syncing');
                    el.textContent = `🔄 Auto-sync ON (${Math.round(detail / 60000)}min)`;
                    el.title = 'Planner auto-sync active — click Import → Planner Live Sync to manage';
                    break;
                case 'success': {
                    el.classList.add('synced');
                    const now = new Date();
                    el.textContent = `✅ Synced ${now.toLocaleTimeString('en', { hour: '2-digit', minute: '2-digit' })}`;
                    el.title = `Last sync: ${now.toLocaleString()} — ${detail} change${detail !== 1 ? 's' : ''}`;
                    // Re-render Gantt and table to reflect merged changes
                    if (detail > 0 && project) { recalculate(); renderAll(); }
                    break;
                }
                case 'error':
                    el.classList.add('error');
                    el.textContent = '⚠️ Sync error';
                    el.title = `Sync error: ${detail} — click Import → Planner Live Sync to retry`;
                    break;
                case 'stopped':
                    if (detail === 'manual') {
                        el.classList.add('hidden');
                        _updatePlannerSyncBtn(false);
                    } else {
                        el.classList.add('error');
                        el.textContent = '❌ Sync stopped';
                        el.title = `Sync stopped: ${detail}`;
                    }
                    break;
            }
        });
    ...
    /**
     * Update the "Planner Live Sync" toolbar button to reflect connection state.
     * connected=true  → green dot + "Planner: Connected"
     * connected=false → original icon + "Planner Live Sync"
     */
    function _updatePlannerSyncBtn(connected) {
        const btn = $('btnPlannerSync');
        if (!btn) return;
        if (connected) {
            btn.textContent = '🟢 Planner: Connected';
            btn.style.color = '#4ade80';
            btn.title = 'Planner is live-synced — click to manage sync settings';
        } else {
            btn.textContent = '🔄 Planner Live Sync';
    ...
    function initPlannerSync() {
        ...
        console.log('[PF] initPlannerSync: btn=', !!btn, 'btnClose=', !!btnClose);
        if (btn)      btn.addEventListener('click', (e) => {
            e.stopPropagation();
            console.log('[PF] Planner Sync button clicked');
            openPlannerSyncModal();
        });
    ...
    function openPlannerSyncModal() {
        ...
        MSGraphClient.renderSetupWizard(body, async ({ planId, planTitle, planIds, planTitles, isPortfolioImport, dataverseUrl }) => {
            ...
            if (isPortfolioImport && planIds && planIds.length > 1) {
                    ...
                    header.textContent = `📊 Importing ${planIds.length} Plans`;
                    header.style.cssText = 'margin:0 0 16px;font-size:16px;color:var(--text-primary,#e2e8f0);';
                    body.appendChild(header);

                    // Progress bar
                    const barWrap = document.createElement('div');
                    barWrap.style.cssText = 'background:rgba(255,255,255,0.08);border-radius:6px;height:8px;margin-bottom:16px;overflow:hidden;';
                    const barFill = document.createElement('div');
                    barFill.style.cssText = 'height:100%;width:0%;background:linear-gradient(90deg,#6366f1,#818cf8);border-radius:6px;transition:width 0.4s ease;';
                    barWrap.appendChild(barFill);
                    body.appendChild(barWrap);

                    // Status text
                    const statusText = document.createElement('div');
                    statusText.style.cssText = 'font-size:12px;color:var(--text-secondary,#9aa0b4);margin-bottom:12px;';
                    statusText.textContent = 'Starting import…';
                    body.appendChild(statusText);

                    // Per-plan status list
                    const planList = document.createElement('div');
                    planList.style.cssText = 'max-height:250px;overflow-y:auto;border:1px solid rgba(255,255,255,0.08);border-radius:8px;background:rgba(0,0,0,0.2);';
                    const planRows = [];
                    planIds.forEach((pid, idx) => {
                        const row = document.createElement('div');
                        row.style.cssText = 'display:flex;align-items:center;gap:10px;padding:8px 12px;font-size:12px;color:var(--text-primary,#e2e8f0);border-bottom:1px solid rgba(255,255,255,0.05);';
                        const icon = document.createElement('span');
                        icon.textContent = '⏳';
                        icon.style.cssText = 'flex-shrink:0;font-size:14px;';
                        const name = document.createElement('span');
                        name.textContent = planTitles[idx] || `Plan ${idx + 1}`;
                        name.style.cssText = 'flex:1;overflow:hidden;text-overflow:ellipsis;white-space:nowrap;';
                        const stat = document.createElement('span');
                        stat.textContent = 'Waiting';
                        stat.style.cssText = 'flex-shrink:0;font-size:11px;color:var(--text-muted,#5c6378);';
                        row.appendChild(icon);
                        row.appendChild(name);
                        row.appendChild(stat);
                        planList.appendChild(row);
                        planRows.push({ row, icon, stat });
                    });
                    ...
                    try {
                        ...
                        const results = await MSGraphClient.importMultipleDataverseProjects(dataverseUrl, planIds, planTitles, (done, total) => {
                            ...
                            if (done <= planRows.length) {
                                ...
                                planRows[done - 1].stat.textContent = 'Done';
                                planRows[done - 1].stat.style.color = '#4ade80';
                            }
                            // Mark next as active
                            if (done < total && done < planRows.length) {
                                planRows[done].icon.textContent = '⏳';
                                planRows[done].stat.textContent = 'Importing…';
                                planRows[done].stat.style.color = '#60a5fa';
                            }
                        });
                        ...
                        for (let ri = 0; ri < results.length; ri++) {
                            ...
                            if (!r.success) {
                                ...
                                if (planRows[ri]) {
                                    ...
                                    planRows[ri].stat.textContent = r.error || 'Failed';
                                    planRows[ri].stat.style.color = '#f87171';
                                }
                                continue;
                            }
                        ...
                        barFill.style.background = failed > 0 ? 'linear-gradient(90deg,#f59e0b,#fbbf24)' : 'linear-gradient(90deg,#22c55e,#4ade80)';
                        statusText.textContent = failed > 0
                            ? `Done: ${saved} imported, ${failed} failed`
                            : `✅ All ${saved} plans imported successfully!`;
                        statusText.style.color = failed > 0 ? '#fbbf24' : '#4ade80';
                        statusText.style.fontWeight = '600';

                        // Close + open portfolio button
                        const doneBtn = document.createElement('button');
                        doneBtn.textContent = '📊 Open Portfolio';
                        doneBtn.style.cssText = 'margin-top:16px;width:100%;padding:10px;border:none;border-radius:8px;background:#6366f1;color:white;font-size:13px;font-weight:600;cursor:pointer;';
    ...
    function _showPlanPickerModal(plans, lastPlanId) {
        ...
        card.innerHTML = `
            <div style="font-weight:600;font-size:0.95rem;margin-bottom:14px;color:var(--text-primary,#e2e8f0);">
                Select a plan to import <span style="font-size:0.75rem;opacity:0.6;font-weight:400;">(${plans.length})</span>
            </div>
