 ...
 */
    ...
    function _layout() {
        ...
        // Check if ANY predecessors exist
        const hasAnyEdges = _tasks.some(t => (t.predecessors || []).length > 0);

        if (hasAnyEdges) {
            // Standard Sugiyama: layer by longest path from sources
            const dfsL  = u => {
            ...
            // No dependencies: arrange by start date → time-based columns
            // Group tasks into time buckets (each week = 1 column)
            const starts = _tasks.map(t => t.start ? new Date(t.start).getTime() : 0).filter(v => v > 0);
            if (starts.length > 0) {
                const minStart = Math.min(...starts);
                const maxStart = Math.max(...starts);
                const range = maxStart - minStart || 1;
                // Target ~8-12 columns for readability
                const maxCols = Math.min(12, Math.max(4, Math.ceil(_tasks.length / 5)));

                _tasks.forEach((t, i) => {
                    const ts = t.start ? new Date(t.start).getTime() : minStart;
                    layer[i] = Math.min(maxCols - 1, Math.floor(((ts - minStart) / range) * (maxCols - 1)));
                });
            }
        }
